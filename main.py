...
for d in [AGENTES_DIR, HISTORY_DIR, AGENDAMENTOS_DIR, IMAGES_DIR, KB_DIR]: d.mkdir(parents=True, exist_ok=True)

# DOCKER: Usa a URL do WAHA do ambiente Docker, senão usa localhost
...
async def waha_send_image(session_name, chat_id, url_imagem, legenda=""):
    async def proc():
        ...
        if espera > 0: await asyncio.sleep(espera)
        try:
            # No Docker, o WAHA precisa acessar a imagem pelo IP do Backend
...
# --- LÓGICA DE IMAGENS (COM DEBUG NO TERMINAL) ---
    ...
    for img in ag.get("imagens", []):
        for tag in img.get("tags", []):
            if tag.lower() in msg_lower:
                url = f"http://localhost:8080/media/{img['arquivo']}"
                print(f"🔍 [DEBUG FOTO] Achou! Mensagem: '{msg_lower}' bateu com a tag '{tag}'")
                return url
                
    print(f"🔍 [DEBUG FOTO] Nenhuma tag bateu para a mensagem: '{msg_lower}'")
    return None
...
# --- MEMÓRIA (AGORA SALVA IMAGENS TAMBÉM) ---
def gerenciar_memoria(agente_id, chat_id, msg_u=None, msg_b=None, type_u="text", type_b="text", url_b=None):
    ...
    if msg_u is not None and msg_b is not None:
        ...
        with open(path, "w") as f: json.dump(hist[-30:], f, ensure_ascii=False, indent=2) # Aumentei para 30
    return hist

def extrair_entidades_e_salvar(texto_resposta, agente_id, chat_id):
    ...
    if datas or horas:
        ...
        hist = json.load(open(path)) if path.exists() else []
        hist.append({"extraido_em": datetime.now(BR_TZ).isoformat(), "datas": datas, "horas": horas})
        with open(path, "w") as f: json.dump(hist, f, ensure_ascii=False, indent=2)
...
async def responder_ia(agente_id, chat_id, mensagem, session_name="default", encontrou_imagem=False):
    ...
    bloco_contexto = f"CONTEXTO DA BASE DE DADOS:\n{contexto_vinculado}" if eh_relevante else "AVISO: NÃO HÁ INFORMAÇÕES SOBRE ISSO NA BASE DE DADOS."
    
    status_imagem = "O SISTEMA ENCONTROU UMA FOTO E ELA ESTÁ SENDO ENVIADA AUTOMATICAMENTE. Fale 'Aqui está!'" if encontrou_imagem else "O CLIENTE PEDIU UMA FOTO, MAS O SISTEMA NÃO ENCONTROU NENHUMA IMAGEM CADASTRADA COM ESSAS PALAVRAS. NUNCA DIGA QUE ESTÁ ENVIANDO FOTO."

    prompt_sistema = f"""{ag['prompt']}\n{sys_vars}\n{bloco_contexto}\nESTADO DE MÍDIA: {status_imagem}\nREGRAS: 1. ANTI-ALUCINAÇÃO. 2. NUNCA ESCREVA [IMAGEM] SE NÃO ENCONTROU. 3. CORRIJA SAUDAÇÃO. 4. BLOCOS CURTOS."""
    ...
    await loop.run_in_executor(executor, extrair_entidades_e_salvar, resposta, agente_id, chat_id)
...
async def waha_webhook(request: Request):
    ...
    async def processar_e_responder():
        ...
        # SALVA NO HISTÓRICO SE ENVIAR FOTO
        if achou_imagem: 
